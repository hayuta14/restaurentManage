...
export class DishService {
  ...
  async create(createDishDto: CreateDishDto) {
    ...
    const tags = await this.dishTagRepository.find({
      where: { id: In(tags_id) },
    });
    dish.tags = tags;
    console.log(dish);
  ...
  async createDishTag(createDishTagDto: CreateDishTagDto) {
    const isExist = await this.dishTagRepository.findOne({
      where: { name: createDishTagDto.name },
    });
    if (isExist) {
  ...
  async deleteDishTag(id: string) {
    const dishTag = await this.dishTagRepository.findOne({
      where: { id: +id },
    });
    if (!dishTag) {
